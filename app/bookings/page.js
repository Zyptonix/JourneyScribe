...
import NavigationBarDark from '@/components/NavigationBarDark'; // Assuming you have this component
...
const QRIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={0.5} d="M5 5h2v2H5V5zm4 0h2v2H9V5zm4 0h2v2h-2V5zm-8 4h2v2H5V9zm4 0h2v2H9V9zm4 0h2v2h-2V9zm-8 4h2v2H5v-2zm4 0h2v2H9v-2zm4 0h2v2h-2v-2zM5 17h2v2H5v-2zm4 1h2v-2H9v2zm4-1h2v2h-2v-2zm4-12h2v2h-2V5zm0 4h2v2h-2V9zm0 4h2v2h-2v-2zm0 4h2v2h-2v-2z" stroke="#fff" /></svg>;
...
const FlightBookingCard = ({ booking }) => {
    // Correctly access nested data from the 'raw' object in Firestore
    ...
    // Find the primary traveler ID from the pricing info
    const travelerId = flightOffer?.travelerPricings?.[0]?.travelerId;
    
    let passengerName = 'Passenger'; // Provide a fallback name

    // Find the traveler in the travelers array that matches the travelerId
    ...
    return (
        <div className="bg-black/20 backdrop-blur-xl border border-white/20 rounded-2xl shadow-lg overflow-hidden flex flex-col md:flex-row my-4">
            ...
            {/* Stub Section */}
            <div className="bg-black/20 p-6 md:border-l-2 md:border-dashed border-white/30 flex flex-col items-center justify-center min-w-[150px]">
                <QRIcon />
                <p className="text-xs text-slate-300 mt-2 text-center">Scan at Gate</p>
            </div>
        </div>
    );
};
...
const HotelBookingCard = ({ booking }) => {
    // This component assumes the hotel data structure is correct and remains unchanged.
    ...
    return (
        <div className="bg-black/20 backdrop-blur-xl border border-white/20 rounded-2xl shadow-lg overflow-hidden my-4 p-6">
...
function BookingsDisplay() {
    ...
    useEffect(() => {
        ...
        const fetchBookings = async () => {
            ...
            setError('');
            try {
                // Fetch Flights - using 'bookings' collection as per previous context
