...
// --- SVG ICONS (Defined in the file as requested) ---
...
function DashboardView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch users and blogs in parallel for efficiency
        ...
        usersSnapshot.forEach(doc => {
          ...
          if (user.createdAt && user.createdAt.toDate) {
            ...
            if (monthDiff >= 0 && monthDiff < 12) {
              const index = 11 - monthDiff; // Index 0 is 11 months ago, 11 is current month
        ...
        for (const location in locationCounts) {
          ...
          if (count > maxCount) {
            maxCount = count;
            popularDestinations = [location]; // New leader
          } else if (count === maxCount) {
            popularDestinations.push(location); // It's a tie
          }
        }
  ...
  // Calculate max chart value for scaling the bars, with a minimum of 50 to prevent huge bars for small numbers
...
function BlogManagementView() {
    ...
    if (loading) return <p className="text-center text-zinc-500">Loading blogs...</p>;
    if (error) return <p className="text-center text-red-500">{error}</p>;
      ...
      <div className="max-w-full min-h-full space-y-6">
        <div className="bg-white p-6 rounded-lg border border-zinc-200 shadow-sm">
          ...
          <form onSubmit={handleSetAdmin}>
            <div className="w-full p-2 mb-4 bg-zinc-100 border border-zinc-200 rounded-md  overflow-y-auto space-y-1">
              {usersLoading ? (
