                ...
                <div className="container mx-auto flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4 md:space-x-6">
                        <div className="hidden md:flex items-center space-x-2">
                                ...
                                {showBookingDropdown && (
                                    <div className="absolute top-full mt-4 w-48 bg-gray-800/[0.8] backdrop-blur-md rounded-lg shadow-xl border border-blue-700 z-30">
                                        <div className="py-2">
                                            ...
                                            <Link href="/bookings" className="block px-4 py-2 text-slate-200 hover:bg-blue-500 hover:text-white transition-colors font-semibold">Confirmations</Link>
                                        </div>
                                    </div>
                                )}
                            </div>
                                ...
                                {showTripsDropdown && (
                                    <div className="absolute top-full mt-4 w-48 bg-gray-800/[0.8] backdrop-blur-md rounded-lg shadow-xl border border-blue-700 z-30">
                                        <div className="py-2">
                                            ...
                                            <Link href="/trips" className="block px-4 py-2 text-slate-200 hover:bg-blue-500 hover:text-white transition-colors font-semibold">Trips</Link> 
                                        </div>
                                    </div>
                                )}
                            </div>
                                ...
                                {showPlanningDropdown && (
                                    <div className="absolute top-full mt-4 w-48 bg-gray-800/[0.8] backdrop-blur-md rounded-lg shadow-xl border border-blue-700 z-30">
                                        <div className="py-2">
                                            ...
                                            <Link href="/activities" className="block px-4 py-2 text-slate-200 hover:bg-blue-500 hover:text-white transition-colors font-semibold">Activities</Link>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            <Link href="/expense" className="text-slate-200 font-semibold px-4 py-2 rounded-full transition-all duration-300 ease-in-out hover:bg-blue-500 hover:text-white hover:shadow-lg">Expenses</Link>
                        ...
                        {loadingAuth ? (
                            ...
                            <div className="relative" ref={userDropdownRef}>
                                <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setShowUserDropdown(prev => !prev)}>
                                    <span className="text-gray-200 text-sm md:text-base font-semibold hidden sm:block">{userNameDisplay}</span>
                                    <img
                                ...
                                {showUserDropdown && (
                                    <div className="absolute top-full right-0 mt-2 w-64 bg-gray-800 rounded-lg shadow-xl border border-blue-700 z-20 overflow-hidden">
                                        {loading && <p className="p-4 text-center text-blue-400">Loading details...</p>}
                                        ...
                                        {!loading && !error && userData && (
                                            <>
                                                <div className="flex items-center gap-3 p-4 border-b border-gray-600">
                                                    ...
                                                    />
                                                    <div>
                                                        <p className="font-semibold text-gray-100 truncate">{userData.fullName || 'N/A'}</p>
                                                        <p className="text-xs text-gray-400 truncate">{userData.email || 'N/A'}</p>
                                                    </div>
                                                </div>
                                                <div className="py-2">
    ...
    <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-2 text-sm text-left ${
            isDestructive 
                ? 'text-red-500' 
                : 'text-gray-300'
        } hover:bg-blue-500 hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed`}
    >
        {icon}
        <span>{children}</span>
    </button>
