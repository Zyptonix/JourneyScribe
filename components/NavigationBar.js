...
export default function NavigationBar() {
  ...
  const handleLogout = async () => {
    ...
    try {
      if (auth) {
        await signOut(auth);
        // The onAuthStateChanged listener will handle resetting user-related states
      } else {
      ...
      // setLoadingAuth will be set to false by the onAuthStateChanged listener
      // once the sign-out process is complete and the user state changes.
      // This ensures the button remains disabled only during the actual sign-out process.
    }
  };
  ...
  const handleUserAreaClick = () => {
    ...
    if (!isUserAuthenticated) {
      ...
      // Fetch user details only when opening the dropdown and if data isn't already loaded or has an error
 ...
 <div className="container mx-auto flex items-center justify-between" style={{ color: 'var(--nav-text-color)' }}>
          ...
          <div className="flex items-center space-x-4 md:space-x-6">
            ...
            {/* Mobile menu button (Hamburger icon or similar) - hidden for this example but good for responsiveness */}
