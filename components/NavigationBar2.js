...
export default function NavigationBar2() {
  ...
  const handleUserAreaClick = () => {
    ...
    if (!isUserAuthenticated) {
      console.log("Sign In button clicked! Redirecting to /auth/login...");
    } else {
  ...
  return (
    <>
      {/* Updated Tailwind classes for a darker, more prominent navbar. */}
      <nav className="relative z-20 w-full bg-gray-800/[0.7] bg-opacity-80 backdrop-blur-md shadow-md">
        <div className="container mx-auto flex items-center justify-between" style={{ color: 'var(--nav-text-color)' }}>
          ...
          <div className="flex items-center space-x-4 md:space-x-6">
            <div className="hidden md:flex items-center space-x-2">
              {/* Updated text color classes to be white for visibility */}
              <Link href="/notifications" className="text-white font-semibold px-4 py-2 rounded-full transition-all duration-300 ease-in-out hover:bg-blue-500 hover:text-white hover:shadow-lg">Notifications</Link>
              <Link href="/Profilepage" className="text-white font-semibold px-4 py-2 rounded-full transition-all duration-300 ease-in-out hover:bg-blue-500 hover:text-white hover:shadow-lg">Profile</Link>
              <Link href="/flight/search" className="text-white font-semibold px-4 py-2 rounded-full transition-all duration-300 ease-in-out hover:bg-blue-500 hover:text-white hover:shadow-lg">Flight</Link>
              <Link href="/hotel/search" className="text-white font-semibold px-4 py-2 rounded-full transition-all duration-300 ease-in-out hover:bg-blue-500 hover:text-white hover:shadow-lg">Hotel</Link>
              <Link href="/blog" className="text-white font-semibold px-4 py-2 rounded-full transition-all duration-300 ease-in-out hover:bg-blue-500 hover:text-white hover:shadow-lg">Blogs</Link>
              <Link href="/chat" className="text-white font-semibold px-4 py-2 rounded-full transition-all duration-300 ease-in-out hover:bg-blue-500 hover:text-white hover:shadow-lg">Chat</Link>
            </div>
            ...
            {loadingAuth ? (
              ...
              <div className="relative flex items-center space-x-3 cursor-pointer" onClick={handleUserAreaClick} ref={dropdownRef}>
                <span className="text-white text-sm md:text-base font-semibold hidden sm:block">
                  {userNameDisplay}
                </span>
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.src = "https://placehold.co/40x40/CCCCCC/666666?text=User";
                  }}
                />
                      ...
                      {!loading && !error && userData && (
                        <div className="space-y-2 text-sm">
                          ...
                          <p className="text-gray-700 dark:text-gray-300 break-words">
                            <span className="font-medium">User ID:</span> <span className="text-xs bg-gray-200 dark:bg-gray-700 p-1 rounded-md">{userId || 'N/A'}</span>
                          </p>
                          {userData.message && (
                            <p className="text-center text-gray-500 dark:text-gray-400 italic mt-4">{userData.message}</p>
                          )}
                        </div>
                      )}
                      {!loading && !error && !userData && (
